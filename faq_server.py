...
async def create_faq(faq: FAQ):
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (faq.theme, faq.main_item, faq.sub_item, faq.detail, faq.content_v1, faq.content_v2, now, now, faq.images, faq.is_archived))
...
async def update_faq(faq_id: int, faq: FAQ):
    ...
    cursor.execute("""
        ...
        WHERE id = ?
    """, (faq.theme, faq.main_item, faq.sub_item, faq.detail, faq.content_v1, faq.content_v2, now, faq.images, faq.is_archived, faq_id))
...
if __name__ == "__main__":
    ...
    try:
        ...
        if count == 0:
            ...
            cursor.executemany("""
                INSERT INTO faqs (theme, main_item, sub_item, detail, content_v1, content_v2, created_date, modified_date, is_archived)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0)
            """, data)
            conn.commit()
        conn.close()
